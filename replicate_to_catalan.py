#!/usr/bin/env python3
"""Replicate CTA optimization from Spanish to Catalan posts."""

import os
import re

# Spanish → Catalan translations for CTA components
ca_translations = {
    # CTA Titles
...
# Get list of Spanish files
es_files = sorted([f for f in os.listdir(es_dir) if f.endswith('.md')])

for es_filename in es_files:
    es_path = os.path.join(es_dir, es_filename)
    ca_path = os.path.join(ca_dir, es_filename)

    if not os.path.exists(ca_path):
        print(f"⚠️  {es_filename} — Catalan version not found")
        continue
    ...
    original_ca = ca_content

    # Extract the CTA from Spanish version (everything after the last >)
    es_cta_match = re.search(r'(>\s*[🔐🛡️🏢📋⏰📊🖥️⚙️📱⚖️🔄🔒♻️🤖🎄📈☁️🎓📜🏫👥🚨🧠☀️🎯📲✅🛍️💰⚠️👁️💡🎁👨‍💼🔧🦠🔌].*?)$', es_content, re.MULTILINE | re.DOTALL)

    if not es_cta_match:
        print(f"⚠️  {es_filename} — Could not extract Spanish CTA")
        continue

    es_cta = es_cta_match.group(1).strip()

    # Translate the CTA to Catalan
    ca_cta = es_cta
    for es_text, ca_text in ca_translations.items():
        ca_cta = ca_cta.replace(es_text, ca_text)

    # Remove old CTA from Catalan version and add new one
    ca_content = re.sub(r'(>\s*[🔐🛡️🏢📋⏰📊🖥️⚙️📱⚖️🔄🔒♻️🤖🎄📈☁️🎓📜🏫👥🚨🧠☀️🎯📲✅🛍️💰⚠️👁️💡🎁👨‍💼🔧🦠🔌].*?)$', '', ca_content, count=1, flags=re.MULTILINE | re.DOTALL)

    ca_content = ca_content.rstrip() + '\n\n' + ca_cta + '\n'
...
print(f"\n✓ Replicated CTA optimization to {updated} Catalan posts")
